    ...
    protected void onImpact(MovingObjectPosition mop) {
        if ((!this.worldObj.isRemote) && (getThrower() != null)) {
            List list = this.worldObj
                    .getEntitiesWithinAABBExcludingEntity(getThrower(), this.boundingBox.expand(1.0D, 1.0D, 1.0D));

            for (int i = 0; i < list.size(); i++) {
                Entity ent = (Entity) list.get(i);
                if (ent instanceof EntityLivingBase) {
                    if (mop.entityHit != null) {
                        ((EntityLivingBase) ent).attackEntityFrom(
                                new EntityDamageSourceIndirect("taint", this, getThrower()).setMagicDamage(),
                                this.damage);
                        if (this.corrosive) {
                            try {
