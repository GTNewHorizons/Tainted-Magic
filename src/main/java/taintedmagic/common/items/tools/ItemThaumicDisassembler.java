...
public class ItemThaumicDisassembler extends Item {
    ...
    public void addInformation(ItemStack s, EntityPlayer p, List l, boolean b) {
        ...
        l.add(
                "\u00A78" + StatCollector.translateToLocal("text.efficiency")
                        + ": "
                        + (getMode(s) == 3 ? "\u00A7c" : "\u00A7a")
                        + getEfficiency(s));
        l.add(" ");
    ...
    public static void renderHUD(ScaledResolution resolution, EntityPlayer player, float partialTicks) {
        ...
        if (s != null && s.getItem() instanceof ItemThaumicDisassembler) {
            String str = "\u00A78" + StatCollector.translateToLocal("text.mode")
                    + ": "
                    + getModeName(s)
                    + (getMode(s) == 3 ? "\u00A7c" : "\u00A7a")
                    + " ("
            ...
            f.drawStringWithShadow(
                    str,
                    resolution.getScaledWidth() / 2 + 100,
                    resolution.getScaledHeight() / 2 + (p.capabilities.isCreativeMode ? 230 : 212),
                    0xFFFFFF);
